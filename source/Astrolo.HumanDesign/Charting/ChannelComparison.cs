...
public sealed class ChannelComparison
{
    ...
    public static ChannelComparison? Evaluate(IChannel channel, PersonalChart lhs, PersonalChart rhs)
    {
        ...
        {
            return new(channel, ChannelMatch.Friendship, new ActiveGate(lStart, true), new ActiveGate(rEnd, true));
        }

        if (lIsComplete)
        {
            return rNotDefined
                ? new(channel, ChannelMatch.Dominant, new ActiveGate(lStart, true), new ActiveGate(lEnd, true))
        ...
        {
            return lNotDefined
                ? new(channel, ChannelMatch.Dominant, new ActiveGate(rStart, false), new ActiveGate(rEnd, false))
        ...
        {
            return new(channel, ChannelMatch.Attraction, new ActiveGate(lStart, true), new ActiveGate(rEnd, false));
        }

        if (lStart.IsActive && rStart.IsActive)
        {
            return new(channel, ChannelMatch.Dominant, new ActiveGate(lStart, true), new ActiveGate(rStart, false));
        }

        if (lEnd.IsActive && rEnd.IsActive)
        {
            return new(channel, ChannelMatch.Dominant, new ActiveGate(lEnd, true), new ActiveGate(rEnd, false));
        }
