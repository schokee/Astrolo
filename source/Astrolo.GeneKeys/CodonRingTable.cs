...
public static class CodonRingTable
{
    ...
    private class CodonRingInfo(CodonRing id, string arcanaArchetype, params int[] sequence)
        : ICodonRing
    {
        public CodonRing Id { get; } = id;

        public string Name => Id.GetDescriptionOrDefault();

        public string ArcanaArchetype { get; } = arcanaArchetype;

        public IReadOnlyList<int> Sequence { get; } = sequence;
