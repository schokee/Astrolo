...
namespace Astrolo.Explorer.UI.Visualisation.Sphere
{
    public static class YiSphere3D
    {
        ...
        public static readonly DependencyProperty HexagramProperty = DependencyProperty.RegisterAttached(
            "Hexagram", typeof(HexagramFigure), typeof(YiSphere3D), new PropertyMetadata(default(HexagramFigure)));
        ...
        public sealed class Geometry
        {
            ...
            {
                FigureSize = new Size3D(iconHeight * 0.9, iconHeight, iconHeight / (6 + SpacingFactor * 5));
        ...
        public static IEnumerable<Visual3D> GenerateVisuals(this ISequence source, Geometry geometry)
        {
            ...
            foreach (var hexagram in source.Hexagrams)
            {
                ...
                for (var i = 0; i < hexagram.LineCount; ++i)
                {
                    ...
                    {
                        trigram.AddBox(new Point3D(0, yMid, 0), figureSize.X, figureSize.Z, figureSize.Z);
                    }
                    else
                    {
                        trigram.AddBox(new Point3D(-xMid, yMid, 0), cx * cxYinLine, figureSize.Z, figureSize.Z);
                        trigram.AddBox(new Point3D(xMid, yMid, 0), cx * cxYinLine, figureSize.Z, figureSize.Z);
                    }
                }
        ...
        public static Transform3D ToTransform(this YiSpherePoint point, double radius)
        {
            ...
            transform.Children.Add(new RotateTransform3D(new AxisAngleRotation3D(new Vector3D(1, 0, 0), point.Elevation.Degrees)));
            transform.Children.Add(new RotateTransform3D(new AxisAngleRotation3D(new Vector3D(0, 0, 1), point.Angle + 90)));
