...
public class HexagramFilter : Filter<HexagramFigure>, IHexagramFilter
{
    ...
    public static IEnumerable<HexagramFilter> Basic
    {
        get
        {
            yield return new("Primary", x => x.HasMatchingTrigrams);
            yield return new("Mirrored", x => x.HasMirroredTrigrams);
            yield return new("Surface", x => !x.Info.SphereLocation.Inside);
            yield return new("Core", x => x.Info.SphereLocation.Inside);
        }
    }
    ...
    public static IEnumerable<HexagramFilter> YinYang
    {
        get
        {
            ...
            {
                var yangCount = i;
                yield return new($"{HexagramInfo.TotalLines - yangCount} Yin / {yangCount} Yang", x => x.Bits.Count == yangCount) { Category = yinYangCategory };
            }

            yield return new("Yang", x => x.IsYang) { Category = yinYangCategory };
            yield return new("Yin", x => x.IsYin) { Category = yinYangCategory };

            yield return new("Upper Yang", x => x.UpperTrigram.IsYang) { Category = yinYangCategory };
            yield return new("Upper Yin", x => x.UpperTrigram.IsYin) { Category = yinYangCategory };

            yield return new("Lower Yang", x => x.LowerTrigram.IsYang) { Category = yinYangCategory };
            yield return new("Lower Yin", x => x.LowerTrigram.IsYin) { Category = yinYangCategory };
        }
    }
    ...
    public static IEnumerable<HexagramFilter> TrigramPairs
    {
        get
        {
            HexagramFilter CreateTrigramFilter(params Trigram[] trigrams)
            {
                ...
                return new(label, x => trigrams.Any(x.HasTrigram)) { Category = "Trigram Combinations" };
            }
    ...
    public static IEnumerable<HexagramFilter> Elements => Enum.GetValues(typeof(Element)).Cast<Element>()
        ...
        .GroupJoin(GateDictionary.Value.SelectMany(x => x.Value.Lines), x => x, x => x.MandalaSlice.SignInfo.Element, (el, matching) =>
        {
            ...
            return new HexagramFilter(el.ToString(), x => lookup.Contains(x.Number))
            {
                Category = "Element",
    ...
    public static IEnumerable<HexagramFilter> ZodiacSigns => Zodiac.AllSigns
        ...
        {
            var lookup = MoreEnumerable.ToHashSet(matching.Select(x => x.Gate.Number));
            return new HexagramFilter(sign.ToString(), x => lookup.Contains(x.Number))
            {
                Category = "Zodiac",
                EmphasizeSelection = true,
            };
    ...
    public static IEnumerable<HexagramFilter> GeneKeys
    {
        get
        {
            yield return new("Delta", x => DeltaPath.Contains(x.Number)) { Category = "Gene Keys" };
        }
    }

    public static IEnumerable<HexagramFilter> Gates => GateDictionary.Value.InTransitOrder
        .Select(x => new HexagramFilter($"{x.Number:D2} {x}", figure => x.Number == figure.Number)
        {
            Category = "Gates",
            EmphasizeSelection = true
        });
    ...
    public static IEnumerable<HexagramFilter> IncarnationCrosses => IncarnationCross
        ...
        .Select(x => new HexagramFilter(x.ToString("l", CultureInfo.CurrentCulture), figure => x.Gates.Contains(figure.Number))
        {
            Category = "Incarnation Cross",
            EmphasizeSelection = true
        });
    ...
    public bool EmphasizeSelection { get; set; }
    ...
    public virtual VisualStates GetState(HexagramFigure hexagram)
    {
        return Includes(hexagram)
            ? EmphasizeSelection ? VisualStates.Emphasized : VisualStates.Selected
            : VisualStates.None;
    }
}
